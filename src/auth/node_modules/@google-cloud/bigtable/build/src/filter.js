...
exports.Filter = exports.FilterError = void 0;
...
class Filter {
    ...
    column(column) {
        ...
        if (col.name) {
            const name = mutation_1.Mutation.convertToBytes(Filter.convertToRegExpString(col.name));
        ...
        if (col.start || col.end) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const range = Filter.createRange(col.start, col.end, 'Qualifier');
    ...
    row(row) {
        ...
        if (r.key) {
            const key = mutation_1.Mutation.convertToBytes(Filter.convertToRegExpString(r.key));
    ...
    value(value) {
        ...
        if (v.value) {
            const valueReg = mutation_1.Mutation.convertToBytes(Filter.convertToRegExpString(v.value));
